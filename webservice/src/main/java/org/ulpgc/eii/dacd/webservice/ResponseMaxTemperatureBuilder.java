...
public class ResponseMaxTemperatureBuilder {
    public List<Response> build() throws SQLException {
        ...
        String sql = "SELECT DATE, TIME, PLACE, MAX(VALUE) FROM maxTemperatures WHERE STATION = ?";
    ...
    public List<Response> buildWithTo(String to) throws SQLException {
        ...
        List<Response> responseList = new ArrayList<>();
        String url = "jdbc:sqlite:database\\datamart.db";
        String sql = "SELECT DATE, TIME, PLACE, MAX(VALUE) FROM maxTemperatures WHERE DATE <= ? AND STATION = ?";
        Connection conn = DriverManager.getConnection(url);
        for (String id : stationIds) {
            PreparedStatement preparedStatement = conn.prepareStatement(sql);
    ...
    public List<Response> buildWithBoth(String from, String to) throws SQLException {
        ...
        List<Response> responseList = new ArrayList<>();
        String url = "jdbc:sqlite:database\\datamart.db";
        String sql = "SELECT DATE, TIME, PLACE, MAX(VALUE) FROM maxTemperatures WHERE DATE BETWEEN ? AND ? AND STATION = ?";
        Connection conn = DriverManager.getConnection(url);
        for (String id : stationIds) {
            PreparedStatement preparedStatement = conn.prepareStatement(sql);
            preparedStatement.setString(1, from);
